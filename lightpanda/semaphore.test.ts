...
describe('createSemaphore', () => {
  ...
  it('never exceeds the limit — the property the memory limit rests on', async () => {
  ...
  it('times a queued waiter out and leaves no phantom slot behind', async () => {
    ...
    // `active` would climb without anyone holding a slot, and the container would eventually
    // run more renders than its memory allows.
    sem.release()
    expect(sem.active).toBe(0)
    expect(await sem.acquire()).toBe(true)
  })
