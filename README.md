...
| `GET /health` | public | — | `{ status: "ok", lastRestartAt, reaped, interrupted }` — only `status` gates anything (Docker healthcheck, rollhook); the counts show an unclean restart to a keyword monitor. Also carries `draining`, `jobs`, the cgroup `memory` ratio and `eventLoopLagMs` / `eventLoopLagPeakMs`. See [Restarts](#restarts-and-what-they-cost) |
...
5 s; a lag over 1 s logs `process.loop_lag` at error level, and `GET /health` exposes both the
latest sample (`eventLoopLagMs`) and the worst of the last 60 s (`eventLoopLagPeakMs`) — the
latest alone is not enough, because a monitor polling every 30-60 s usually reads the quiet
interval that followed the stall. One Bun process serves everything — listener, heartbeats and
...
aborts live workers. 2026-09-20 produced exactly that shape. The structural fix is the pair in
`response-kind.ts` / `fetch-chain.ts`: `MAX_BODY_BYTES` bounds what the fetch chain DOWNLOADS,
through a byte-counting reader that cancels the response once it is over, and `PARSE_INPUT_CAP`
bounds what it PARSES (linkedom/Readability run synchronously on the shared loop). Both over-cap
cases are a miss like any other — the chain falls through to lightpanda and Tavily Extract, its
normal fallbacks. If `process.loop_lag` still fires with both in place, the next step is
offloading the parse to a Bun Worker.
...
- `GET /health` carries `lastRestartAt`, `reaped` (this boot), `interrupted` (this process
  ...
  `eventLoopLagMs` / `eventLoopLagPeakMs` — enough
