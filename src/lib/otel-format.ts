...
/**
 * `OTEL_EXPORTER_OTLP_HEADERS` → header map; same `key=value,key=value` grammar as
 * `parseResourceAttributesEnv` in otel.ts, except values MAY contain `=` (e.g. a base64
 * token) — only the FIRST `=` splits key from value. Ported from audio-gateway's
 * `parseOtlpHeadersEnv`. A pair with no `=`, an empty key, or an empty value is skipped
