...
| Has the owner already looked into this, decided something about it, or saved reading about it | \`brainNotes\` — the owner's notes and Karakeep bookmarks (when available); \`brainNotes({ path })\` reads a note the question names (\"Areas/…/Rammus\", \"my Rammus note\") in full | re-deriving a conclusion from scratch; \`fetchPage\` on a brain URL |
