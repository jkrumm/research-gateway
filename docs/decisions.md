...
| Models | DeepSeek-V4-Pro leads, V4-Flash works | → **Flash for both** once synthesis became the wall-clock long pole → **`gpt-5.6-luna` for both** (2026-08-20): 3-8x faster to first token, 100% tool-calling reliability, comparable reports → **`deepseek-v4.1-flash` for both** (2026-09-13), `reasoning_effort: "high"`: an estate-wide model decision, not a per-service latency re-measurement — accepts no prompt-cache discount on this route as a known cost. `gpt-6-luna` was deployed for both on 2026-09-23 and reverted the same day: Luna takes function tools only at `reasoning_effort: "none"`, a research-quality downgrade against capability-over-cost. Effort + per-call-role output budget (plan/worker-step/synthesis) live in `src/lib/llm.ts`; rationale in `src/env.ts` and modelpick |
