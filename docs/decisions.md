...
| Job store | in-memory v1 | → **`bun:sqlite`, status-only durability, heartbeat-reaped** (superseded below) — a restart no longer 404s every job; a job caught mid-run came back as a terminal `error`. Heartbeat, not "everything running at boot is dead", because rollhook's overlap runs two replicas on the same file |
